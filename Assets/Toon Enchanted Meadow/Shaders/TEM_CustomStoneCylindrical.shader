...
Shader "Toon/TEM_CustomStoneCylindrical"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float _MainNormalMapScale;
			float _OverlayTextureScale;
			float _OverlayNormalMapScale;
			float _CovOffset;
			float _OcclusionIntensity;
			float _EdgeHighlights;
						 ...
						 ) : SV_Target
			{
				...
				float2 MainTextureTiling172 = ( texCoord42 * _MainTextureTiling );
				float MainNormalMapScale258 = _MainNormalMapScale;
				float3 unpack52 = UnpackNormalScale( tex2D( _MainNormalMap, MainTextureTiling172 ), MainNormalMapScale258 );
				unpack52.z = lerp( 1, unpack52.z, saturate(MainNormalMapScale258) );
				float OverlayTextureTiling218 = ( 1.0 / _OverlayTextureScale );
				float2 temp_cast_0 = (OverlayTextureTiling218).xx;
				...
				float3 temp_cast_1 = (OverlayNormalMapScale261).xxx;
				float3 triplanar217 = TriplanarSampling217( _OverlayNormalMap, _MainNormalMap, _MainNormalMap, WorldPosition, ase_normalWS, 8.0, temp_cast_0, temp_cast_1, float3(0,0,0) );
				float3 tanTriplanarNormal217 = mul( ase_worldToTangent, triplanar217 );
				float dotResult63 = dot( float3(0,1,0) , ase_normalWS );
				float2 uv_MainNormalMap = input.ase_texcoord6.xy * _MainNormalMap_ST.xy + _MainNormalMap_ST.zw;
				float3 tanToWorld0 = float3( ase_tangentWS.x, ase_bitangentWS.x, ase_normalWS.x );
				float3 tanToWorld1 = float3( ase_tangentWS.y, ase_bitangentWS.y, ase_normalWS.y );
				float3 tanToWorld2 = float3( ase_tangentWS.z, ase_bitangentWS.z, ase_normalWS.z );
				float3 tanNormal324 = UnpackNormalScale( tex2D( _MainNormalMap, uv_MainNormalMap ), 1.0f );
				...
				float OverlayCoverage340 = saturate( ( ( worldNormal324.y + (-1.0 + (_CovOffset - 0.0) * (2.0 - -1.0) / (1.0 - 0.0)) ) * ( 1.0 - tex2D( _DetailMask, uv_DetailMask329 ).a ) ) );
				float clampResult317 = clamp( ( dotResult63 * OverlayCoverage340 ) , 0.0 , 1.0 );
				float3 lerpResult280 = lerp( ( unpack52 * ( float3(0,0,1) + float3(1,1,0) ) ) , ( tanTriplanarNormal217 * ( float3(1,1,0) + float3(0,0,1) ) ) , clampResult317);
				float3 BlendedNormals169 = lerpResult280;
				float3 tanNormal145 = BlendedNormals169;
				float3 worldNormal145 = normalize( float3( dot( tanToWorld0, tanNormal145 ), dot( tanToWorld1, tanNormal145 ), dot( tanToWorld2, tanNormal145 ) ) );
				float3 worldNormalValue185_g5 = worldNormal145;
				...
				float3 temp_output_364_0 = localAdditionalLightsLambertMask14x136_g5;
				float3 bakedGI142 = ASEIndirectDiffuse( input, ase_normalWS );
				Light ase_mainLight = GetMainLight( ShadowCoords );
				MixRealtimeAndBakedGI( ase_mainLight, ase_normalWS, bakedGI142, half4( 0, 0, 0, 0 ) );
				float ase_lightIntensity = max( max( _MainLightColor.r, _MainLightColor.g ), _MainLightColor.b ) + 1e-7;
				float4 ase_lightColor = float4( _MainLightColor.rgb / ase_lightIntensity, ase_lightIntensity );
				float4 tex2DNode45 = tex2D( _MainTexture, MainTextureTiling172 );
				float2 uv_DetailMask = input.ase_texcoord6.xy * _DetailMask_ST.xy + _DetailMask_ST.zw;
				...
				float2 temp_cast_4 = (OverlayTextureTiling218).xx;
				float4 triplanar215 = TriplanarSampling215( _OverlayTexture, _MainTexture, _MainTexture, WorldPosition, ase_normalWS, 8.0, temp_cast_4, float3( 1,1,1 ), float3(0,0,0) );
				float4 lerpResult26 = lerp( lerpResult224 , triplanar215 , OverlayCoverage340);
				float4 BlendedTextures167 = lerpResult26;
				float dotResult146 = dot( worldNormal145 , _MainLightPosition.xyz );
				float temp_output_149_0 = (dotResult146*0.49 + 0.5);
				float2 temp_cast_5 = (temp_output_149_0).xx;
				float ase_lightAtten = 0;
				ase_lightAtten = ase_mainLight.distanceAttenuation * ase_mainLight.shadowAttenuation;
				float3 temp_output_361_0 = ( temp_output_364_0 + ( ase_lightAtten * ase_lightColor.rgb * ase_lightColor.a ) );
				float3 break147 = temp_output_361_0;
				float3 temp_cast_7 = (1.0).xxx;
				#ifdef UNITY_PASS_FORWARDBASE
				float3 staticSwitch157 = temp_cast_7;
				#else
				float3 staticSwitch157 = temp_output_361_0;
				#endif
				float4 CustomLighting109 = ( ( float4( ( temp_output_364_0 + ( bakedGI142 * ase_lightColor.rgb * ase_lightColor.a ) ) , 0.0 ) * BlendedTextures167 ) + ( ( ( tex2D( _TextureRamp, temp_cast_5 ) * float4( ( ase_lightColor.rgb + bakedGI142 ) , 0.0 ) ) * ( temp_output_149_0 * max( max( break147.x , break147.y ) , break147.z ) ) ) * ( BlendedTextures167 * float4( staticSwitch157 , 0.0 ) ) ) );
				
				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = CustomLighting109.xyz;
				float Alpha = 1;
...
Node;AmplifyShaderEditor.CommentaryNode;185;-880,-4880;Inherit;False;2436;1091;Comment;30;141;140;143;144;145;146;147;149;150;142;154;148;155;158;159;156;162;161;181;163;164;165;166;109;214;157;153;361;363;364;Custom Lighting;1,1,1,1;0;0
...
Node;AmplifyShaderEditor.TexturePropertyNode;198;3280,-6560;Inherit;True;Property;_DetailMask;Detail Mask;3;0;Create;True;0;0;0;False;0;False;None;69a26c6eec8616a4f9b40cddf50ac75a;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;144;-560,-4496;Inherit;False;3;3;0;FLOAT;0;False;1;FLOAT3;0,0,0;False;2;FLOAT;0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.ScaleAndOffsetNode;149;-96,-4720;Inherit;False;3;0;FLOAT;1;False;1;FLOAT;0.49;False;2;FLOAT;0.5;False;1;FLOAT;0
...
WireConnection;361;1;144;0
WireConnection;366;0;365;0
WireConnection;365;0;364;0
WireConnection;351;2;170;0
ASEEND*/
//CHKSM=08BDA7B38F0B85F79B0D46F67FCAD984FDD63992